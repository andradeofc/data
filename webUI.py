...
def process_segment(video_seg, audio_seg, i):
    # Definindo o caminho do diretório para onde você quer mudar
    directory_path = "/content/video-retalking"
    ...
    # Nota: Aqui, você não precisa mais do caminho completo para inference.py,
    # já que você está no diretório correto
    command = ["python", "inference.py", "--face", video_seg,
               "--audio", audio_seg, "--outfile", output_file]
    
    subprocess.run(command, check=True)

    # Retorna para o diretório original, se necessário
...
with gradio.Blocks(
    ...
    with gradio.Row():
        with gradio.Column():
            ...
            with gradio.Row():
                btn = gradio.Button(value="Executar CloneX",variant="primary")
            with gradio.Row():
                gradio.Examples(
                    label="Videos Exemplos",
                    examples=[
